...
This requires compilation. Windows users can download the lates compiled version from appveyor [here](https://ci.appveyor.com/project/gvegayon/ergmito/build/artifacts). The file to download is the one named `ergmito_[version number].zip`. Once donwloaded, you can install typing the following:
...
The pooled model can be also compared to a block-diagnoal ERGM. The package includes
...
```{r, echo=FALSE}
t_ergm <- structure(c(user.self = 5.076, sys.self = 0.056, elapsed = 5.134, 
user.child = 0.016, sys.child = 0.016), class = "proc_time")
t_ergmito <-structure(c(user.self = 0.0599999999999987, sys.self = 0.00800000000000001, 
elapsed = 0.0660000000000025, user.child = 0, sys.child = 0), class = "proc_time")
```
...
Suppose that we have a large sample of small networks (ego from facebook, twitter, etc.), 2,000:
