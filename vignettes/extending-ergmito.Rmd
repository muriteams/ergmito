...
# Printing the loglikelihood function
model_object$loglik
```

Notice that, while the function `ergmito_formulae` returns all the components 
needed to compute the joint likelihood, the function still needs all the parameters
to be passed explicitly. The description of the parameters, which is also available
in the package manual, is:

- `params` a numeric vector with the parameters for the sufficient statistics
- `stats.weights` a list of vectors. The weights that go along the sufficient
statistics on the support.
- `stats.statmat` a list of matrices with the sufficient statistics on the
support of the function.
- `target.stats` a matrix with the observed sufficient statistics.

We can take a look at each component from our previous object:
...
logposterior <- function(p) {
  model_object$loglik(
    params = p,
    stats.weights = model_object$stats.weights,
    stats.statmat = model_object$stats.statmat,
    target.stats = model_object$target.stats
  ) + 
...
For this example, we are using an R package that is yet to be submitted to CRAN
(already published in the Journal of Open Source Software), fmcmc. To install it
you can type:
...
Here is how we put everything together:
